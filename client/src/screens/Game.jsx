...
// ---------- เอฟเฟกต์ gif ทับไอคอนผู้เล่น (patch 3.4 — ผู้วิงวอน) ----------
//  ต่างจาก cutscene ตรงที่ "ไม่หยุดเกม": server ยิง event iconFx มา แล้วเราวาด gif ทับพอร์เทรตของคนนั้นชั่วครู่
//  เก็บ state ไว้นอก React (module scope) เพราะ Portrait ถูก mount หลายที่พร้อมกัน (โต๊ะรวม/การ์ดเล็ก/โมดัล)
//  การยัด state ไว้ที่ component เดียวแล้ว prop-drill ลงไปทุกจุดจะต้องแก้ call site เป็นสิบแห่ง
const iconFxListeners = new Set();
let iconFxState = {}; // { [playerId]: { gif, seq } }
function pushIconFx(fx) {
  if (!fx || !fx.targetId || !fx.gif) return;
  iconFxState = { ...iconFxState, [fx.targetId]: { gif: fx.gif, seq: fx.seq } };
  iconFxListeners.forEach((fn) => fn(iconFxState));
  setTimeout(() => {
    // ถ้ามีเอฟเฟกต์ใหม่ทับเข้ามาแล้ว (seq ไม่ตรง) ปล่อยให้อันใหม่คุมเวลาของตัวเองไป
    if (!iconFxState[fx.targetId] || iconFxState[fx.targetId].seq !== fx.seq) return;
    const next = { ...iconFxState };
    delete next[fx.targetId];
    iconFxState = next;
    iconFxListeners.forEach((fn) => fn(iconFxState));
  }, fx.ms || 2000);
}
function useIconFx(playerId) {
  const [state, setState] = useState(iconFxState);
  useEffect(() => {
    const fn = (next) => setState(next);
    iconFxListeners.add(fn);
    return () => { iconFxListeners.delete(fn); };
  }, []);
  return playerId ? state[playerId] : null;
}
...
function Portrait({ p, className, rounded = "rounded-2xl" }) {
  ...
  return (
    <div className={`relative ${aura} ${className}`}>
      ...
      {/* ตราพิพากษา (ผู้วิงวอน): gif ค้างบนไอคอนเป้าหมายจนกว่าตราจะหมด — จางกว่าเอฟเฟกต์ชั่วครู่เพื่อไม่บังหน้า */}
      {p.supJudge?.gif && (
        <img src={p.supJudge.gif} alt="" aria-hidden="true"
          className={`absolute inset-0 w-full h-full object-cover pointer-events-none opacity-60 ${rounded}`} />
      )}
      {/* เอฟเฟกต์ชั่วครู่ (เยียวยา/เกราะศรัทธา/คำพิพากษา) — key={seq} บังคับให้ gif เริ่มเล่นใหม่ทุกครั้ง */}
      {iconFx?.gif && (
        <img key={iconFx.seq} src={iconFx.gif} alt="" aria-hidden="true"
          className={`absolute inset-0 w-full h-full object-cover pointer-events-none z-10 ${rounded}`} />
      )}
    ...
    <span className={`inline-flex items-center gap-1 whitespace-nowrap shrink-0 ${className}`}>
      ...
      {p.supFaith > 0 && (
        <span
          ...
          title={`เกราะศรัทธา ${p.supFaith}/${p.supFaithMax || 3} — เกราะพิเศษที่กินความเสียหายก่อนเกราะหลักเสมอ (ยกเว้นการโจมตีทะลุเกราะ) · ระหว่างที่ยังเหลือ เจ้าของได้ "คุ้มครอง 1" และ "เสริมพลัง 1"`}
        >
          {"🔰".repeat(p.supFaith)}
        </span>
      )}
      {/* โปรดิวเซอร์ (patch 3.6): หลอดที่วาดด้านบนคือของ "ไอดอล" — ต่อท้ายด้วยเลือดโปรดิวเซอร์ที่ยืนหลัง
          เพื่อให้เห็นชัดว่าเหลือกันชนอีกกี่หน่วยก่อนตกรอบจริง (ไอดอลล้มแล้วหลอดหลักจะกลายเป็นของโปรดิวเซอร์เอง) */}
      {p.lumiProducerHp != null && !p.lumiIdolDown && (
        <span
          className={`${sm ? "text-[10px]" : "text-xs"} font-bold text-echo-magenta whitespace-nowrap`}
          title={`โปรดิวเซอร์เหลือพลังชีวิต ${p.lumiProducerHp}/${p.lumiProducerMax} — ไอดอลล้มเมื่อไหร่ ความเสียหายจะตกมาที่นี่`}
        >
          🎧{p.lumiProducerHp}
        </span>
      )}
...
// ผู้วิงวอน: ชื่อท่าที่โชว์ในแถบ "แตะเลือกเป้าหมาย" ตาม tier ที่กำลังรออยู่
const SUP_SKILL_LABEL = { basic: "Prayer", secondary: "Armor of Faith", ultimate: "Mark of Judgment" };
...
const STATUS_INFO = {
  ...
  eijiUlt: { icon: "🔥", label: "ไม่ว่ายังก็ตาม", cls: "bg-echo-gold text-gray-900", desc: "ไม่ว่ายังก็ตาม: บังคับเปิดสนาม Break Beat Bark! — ทุกคนได้พลังโจมตีปกติ +1 · เวลาเฟสจั่วการ์ดเหลือ 40 วินาที · เอจิหลบหลีก +20% และได้แต้มสกิล +1 ต่อเทิร์น · หมดเวลาแล้วติดคูลดาวน์ 3 เทิร์น (ตัวเลขนับถอยหลังขึ้นทับบนการ์ดสกิล)" },
  ...
  muimiTower: { icon: "⚔️", label: "ดาบสะบั้น", cls: "bg-echo-gold text-gray-900", desc: "ดาบสะบั้น: พลังโจมตีพื้นฐาน +3 และเมื่อโจมตีปกติจะฟื้นพลังชีวิต 2 หน่วย พร้อมยืดเวลาสถานะออกไปอีก 1 เทิร์น — ระหว่างสถานะนี้ Overload Force จะไม่เกิด" },
  ...
  lumiLuminous:  { icon: "🌈", label: "luminous", cls: "bg-echo-gold text-gray-900", desc: "luminous: รวมความสามารถของท่าไม้ตายทั้ง 5 แบบและผลติดตัวของไอดอลทั้ง 5 คนเข้าด้วยกัน (พลังโจมตี +1 ไม่ซ้อนทับ) — ถูกโจมตีครบตามจำนวนคู่ต่อสู้ที่ยังอยู่ (นับจำนวนครั้ง ไม่ใช่จำนวนคน) จะฟื้นพลังชีวิต 2 และเกราะ 1" },
  // ---------- ไบรอัน (GT-R34) (patch 3.5 new) ----------
  brianCar:    { icon: "🚗", label: "รถคู่ใจ", cls: "bg-echo-cyan text-gray-900", desc: "รถคู่ใจ: กินน้ำมันเทิร์นละ 1 หน่วย และทุกๆ 2 หน่วยที่รถกินจะฟื้นพลังชีวิต +1 — อยู่จนกว่าน้ำมันจะหมดถังหรือกดดับเครื่องเอง · ต้องอยู่ในร่างนี้ถึงจะใช้สกิลรองและท่าไม้ตายได้" },
  ...
  brianPush:   { icon: "🏎️", label: "หลีกทางไป", cls: "bg-echo-gold text-gray-900", desc: "หลีกทางไป: ทุกเทิร์นหลังเปิดไพ่ จะพุ่งชนผู้เล่นที่แต้มสูงสุดในบรรดาคนที่แต้มมากกว่าเรา สร้างความเสียหาย 1 หน่วย (2 หน่วยในร่างเพิ่มพลัง) — ระหว่างนี้ใช้ท่าไม้ตายไม่ได้" },
  ...
  cayGepard: { icon: "🛡️", label: "เกพาร์ด", cls: "bg-echo-gold text-gray-900", desc: "ร่วมร่างสหายแห่งเทพ (คาเยนน์): ร่างเกพาร์ด — ความเสียหายที่ได้รับไม่ถูกเลื่อนอีกต่อไป · การโจมตีปกติแต่ละนัดที่เข้าเป้ามีโอกาส 50% มอบ \"เปราะบาง\" · ปลดล็อก \"แน่จริงก็หลบสิ\" และ \"มิสไซล์แห่งคำอำลา\"" },
  ...
  supFaith:  { icon: "✝️", label: "เกราะศรัทธา", cls: "bg-echo-gold text-gray-900", desc: "เกราะศรัทธา: เกราะพิเศษที่เสริมขึ้นมา (สะสมได้สูงสุด 3 หน่วย · 1 หน่วย = เกราะ 1 หน่วย) — ความเสียหายทุกช่องทาง รวมดาเมจนอกสนามและดาเมจจากสถานะดีบัฟ จะกินเกราะนี้ก่อนเกราะหลักเสมอ ยกเว้นการโจมตีทะลุเกราะ · ปืนสลายเกราะทำลายได้ · การโจมตีที่คำนวณแล้วดาเมจเหลือ 0 ทำลายไม่ได้ · ตราบที่ยังเหลืออยู่ เจ้าของได้รับ \"คุ้มครอง 1\" และ \"เสริมพลัง 1\" ตลอดเวลา · ล้าง/ต้านไม่ได้" },
...
function statusEntries(p, full) {
  ...
  if ((p.shidoGuard || 0) > 0) {
    out.push({ key: "shidoGuardCd", v: p.shidoGuard, icon: "🕰️", label: `ฝากด้วยนะตัวฉัน ${p.shidoGuard} เทิร์น`, cls: "bg-echo-cyan text-gray-900", desc: "ฝากด้วยนะตัวฉัน: กับดักเปิดอยู่ — คุณเห็นตัวนับนี้อยู่คนเดียว ผู้เล่นคนอื่นไม่รู้เลยว่าคุณกดท่าไม้ตายไปแล้ว (พวกเขายังเห็นแต้มสกิลของคุณเต็มหลอด) · ถ้าตกรอบระหว่างนี้ เวลาทั้งสนามจะย้อนกลับไป 5 เทิร์นก่อนหน้า ทุกอย่างกลับเป็นเหมือนเดิม รวมทั้งคนที่ตกรอบไปแล้ว" });
  }
  if ((p.shidoCd || 0) > 0) {
  ...
  // โปรดิวเซอร์: ไอดอลที่ยืนแนวหน้า + แต้ม "ไอดอล" (ทุกคนเห็นได้ — เป็นข้อมูลสนามที่ต้องอ่านออก)
  if (p.character?.id === "producer_lumi") {
    out.push(p.lumiIdolDown
      ? { key: "lumiIdol", v: 1, icon: "💔", label: "ไอดอลล้ม", cls: "bg-echo-hp", desc: "ไอดอลล้มแล้ว — ความเสียหายตกมาที่โปรดิวเซอร์ (พลังชีวิต 3) · ใช้ \"ลุกขึ้นมา ไอดอลที่ฉันภาคภูมิใจ\" เพื่อชุบกลับ" }
  ...
  // คาเยนน์: กระสุน / แรงใจ / ชุดกระสุนที่บรรจุไว้ / ความเสียหายที่เลื่อนไว้ (ทุกคนเห็นได้ — เป็นข้อมูลสนาม)
  if (p.cayenne) {
    const c = p.cayenne;
    ...
    if (c.barrage) out.push({ key: "cayBarrage", v: 1, icon: "🎯", label: "แน่จริงก็หลบสิ (บรรจุแล้ว)", cls: "bg-echo-magenta", desc: "การโจมตีปกติครั้งถัดไปยิง 4 นัด นัดละ 1 หน่วย (นัดที่ 4 มีโอกาส 50%) — ค้างไว้จนกว่าจะได้โจมตี" });
  ...
  // ไบรอัน: น้ำมันเป็นข้อมูลสาธารณะ — เป็นตัวจับเวลาของร่างรถที่ทุกคนต้องอ่านออกว่าเหลืออีกกี่เทิร์น
  if (p.character?.id === "brian") {
    const perTurn = p.brianBoost ? 2 : 1;
    out.push({
      key: "brianFuel", v: 1, icon: "⛽", label: `น้ำมัน ${p.brianFuel || 0}/${p.brianFuelMax || 12}`,
...
// ---------- โปรดิวเซอร์: เลือกไอดอลที่จะสลับมายืนแนวหน้า ----------
//  ต้องเป็นโมดัลเพราะมี 5 ตัวเลือกพร้อมคำอธิบายผลติดตัว — เลือกบนกระดานไม่ได้ (ไม่ใช่การเล็งผู้เล่น)
function LumiIdolModal({ me, onPick, onClose }) {
  const list = me.lumiIdols || [];
  return (
    <div className="fixed inset-0 z-40 bg-black/60 grid place-items-center p-4" onClick={onClose}>
      <div className="bg-echo-navy rounded-2xl p-5 max-w-lg w-full shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-baseline gap-2 mb-3">
          <span className="text-lg font-black text-echo-cyan">🎙️ ฉันเชื่อมั่นในตัวเธอนะ</span>
          <span className="text-xs opacity-60">สลับไอดอลมายืนแนวหน้า</span>
        </div>
        <div className="grid gap-2">
          {list.map((it) => (
            <button
              ...
              disabled={it.current}
              onClick={() => { clickSound(); onPick(it.key); }}
              className={`flex items-center gap-3 text-left rounded-lg border-2 px-3 py-2 transition ${
                it.current ? "opacity-45 cursor-not-allowed border-white/10" : "border-white/15 hover:border-echo-cyan"
              }`}
            >
              <img src={it.img} alt="" className="w-11 h-11 rounded-lg object-cover shrink-0" />
              <span className="min-w-0">
                <span className="font-bold flex items-center gap-2">
                  {it.name}
                  {it.current && <span className="text-[11px] px-1.5 py-0.5 rounded bg-white/15">ยืนอยู่ตอนนี้</span>}
                </span>
                <span className="block text-xs opacity-75 mt-0.5">{it.passive}</span>
              </span>
            </button>
          ))}
        </div>
        <div className="text-[11px] opacity-55 mt-3">สลับ 1 ครั้ง = โปรดิวเซอร์ฟื้นพลังชีวิต +1 · ท่าไม้ตายจะเปลี่ยนตามไอดอลที่เลือก</div>
        <button onClick={onClose} className="w-full mt-3 rounded-lg py-2 font-bold bg-black/50 border border-white/25">ยกเลิก</button>
      </div>
    </div>
  );
}
...
function BrianKeyModal({ me, onPick, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-40 bg-black/60 grid place-items-center p-4" onClick={onClose}>
      <div className="bg-echo-navy rounded-2xl p-5 max-w-sm w-full shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="text-lg font-black text-echo-cyan mb-1">🔑 กุญแจรถ</div>
        <div className="text-xs opacity-70 mb-3">
          ตอนนี้อยู่ในร่าง <b>{boosted ? "รถคู่ใจที่ขาดไม่ได้" : "รถคู่ใจ"}</b> · น้ำมันเหลือ {me.brianFuel}/{me.brianFuelMax}
        </div>
        <div className="grid gap-2">
          <button
            onClick={() => { clickSound(); onPick("off"); }}
            className="text-left rounded-lg border-2 border-white/15 hover:border-white/40 px-3 py-2 transition"
          >
            <div className="font-bold">🅿️ ดับเครื่อง</div>
            <div className="text-xs opacity-75 mt-1">ลงจากรถ — หยุดกินน้ำมัน และกลับมาเติมน้ำมันเทิร์นละ 1 หน่วยได้ตามปกติ</div>
          </button>
          <button
            disabled={boosted}
            onClick={() => { clickSound(); onPick("boost"); }}
            className={`text-left rounded-lg border-2 px-3 py-2 transition ${
              boosted ? "opacity-40 cursor-not-allowed border-white/10" : "border-echo-hp hover:border-white/60"
            }`}
          >
            <div className="font-bold">🔥 เพิ่มพลัง {boosted && <span className="text-[11px] px-1.5 py-0.5 rounded bg-echo-hp ml-1">เปิดอยู่แล้ว</span>}</div>
            <div className="text-xs opacity-75 mt-1">เปลี่ยนเป็น &quot;รถคู่ใจที่ขาดไม่ได้&quot; — กินน้ำมันเทิร์นละ 2 หน่วยแทน 1 แลกกับพลังโจมตีพื้นฐาน +1</div>
          </button>
        </div>
        <button onClick={onClose} className="w-full mt-3 rounded-lg py-2 font-bold bg-black/50 border border-white/25">ยกเลิก</button>
      </div>
...
export default function Game({ state, lowQ, skillConfirmOn = true }) {
  ...
  // อิสึกะ ชิโด: ตัวเลขที่โชว์ทับบนการ์ดท่าไม้ตาย (เจ้าของเห็นคนเดียว — server ส่งสองค่านี้ให้เฉพาะ mine)
  //  กับดักเปิดอยู่ = นับถอยหลังของกับดัก · เพิ่งย้อนเวลาไป = นับถอยหลังคูลดาวน์ห้ามกดซ้ำ
  //  ทั้งสองกรณีปุ่มถูกล็อกอยู่แล้ว ตัวเลขจึงอธิบายให้ว่า "ทำไมกดไม่ได้" และ "อีกกี่เทิร์นถึงกดได้"
  ...
  // อิปโป: คูลดาวน์รายสกิลทั้งสามช่อง (server ส่งเทิร์นที่เหลือมาให้) โชว์เป็นตัวเลขทับบนการ์ดเหมือนของเอจิ/ชิโด
  const ippoCd = ch?.id === "ippo" ? (me?.ippoCd || {}) : {};
  const ippoBasicCd = ippoCd.basic || 0;
  const ippoSecCd = ippoCd.secondary || 0;
  ...
  // ไบรอัน: สกิลรอง/ท่าไม้ตายต้องอยู่ในร่างรถและมีน้ำมันพอ · ระหว่างการแข่งช่องท่าไม้ตายเป็น N2O
  const isBrian = ch?.id === "brian";
  const brianCarOn = isBrian && !!me?.brianCar;
  const brianFuel = isBrian ? (me?.brianFuel || 0) : 0;
  const brianSecLocked = isBrian && (!brianCarOn || brianFuel < 2 || (me?.statuses?.brianPush || 0) > 0);
  // N2O ระหว่างการแข่ง: ต้องข้ามด่าน me.skillUsed ของปุ่มด้วย เพราะท่าไม้ตาย 1 กินโควตาไปแล้ว
  //  ในเทิร์นเดียวกัน (server ยกเว้นให้แล้วที่ isBrianN2O ของ useSkill — ฝั่งปุ่มต้องตรงกัน)
  const isBrianN2O = isBrian && !!me?.brianN2O;
  const brianUltLocked = isBrian && (me?.brianN2O
    ? brianFuel < 4                                              // N2O: ต้องมีน้ำมันอย่างน้อย 4
    : (!brianCarOn || brianFuel < 2 || (me?.statuses?.brianPush || 0) > 0));
  // โปรดิวเซอร์: ช่องแรกไม่กินโควตาสกิล (ต้องยกเว้นเหมือนกุญแจรถ) · ระหว่างท่าไม้ตายสลับไอดอลไม่ได้
  //  ท่าไม้ตาย/สกิลรองกดไม่ได้ตอนไอดอลล้ม (เหลือแต่ช่องชุบ) — ต้องตรงกับ canUseSkill ฝั่ง server
  const isLumi = ch?.id === "producer_lumi";
  const lumiIdolDown = isLumi && !!me?.lumiIdolDown;
  const lumiUltOn = isLumi && ((me?.statuses?.lumiUlt || 0) > 0 || (me?.statuses?.lumiLuminous || 0) > 0);
  const lumiBasicLocked = isLumi && !lumiIdolDown && lumiUltOn;
  const lumiSecLocked = isLumi && (lumiIdolDown || (me?.statuses?.lumiTrain || 0) > 0);
  const lumiUltLocked = isLumi && (lumiIdolDown || lumiUltOn);
  // คาเยนน์: ปืนพก 1 ครั้ง/เทิร์น (ไม่กินโควตาสกิล) · รอง/ท่าไม้ตายต้องเป็นเกพาร์ด + กระสุนพอ — ต้องตรงกับ canUseSkill ฝั่ง server
  const isCay = ch?.id === "cayenne";
  const cayState = (isCay && me?.cayenne) || {};
  const cayBasicLocked = isCay && cayState.pistolRound === state.roundNumber;
  const caySecLocked = isCay && (!cayState.gepard || !!cayState.barrage || (cayState.ammo || 0) < 1);
  const cayUltLocked = isCay && (!cayState.gepard || (cayState.ammo || 0) < 2);
  ...
  useEffect(() => {
    const onIconFx = (fx) => { pushIconFx(fx); if (fx?.sound) playSfx(fx.sound); };
    socket.on("iconFx", onIconFx);
    return () => socket.off("iconFx", onIconFx);
  }, []);
  useEffect(() => {
    if (!notice) return;
  ...
  // ไบรอัน: เลือกคู่แข่งของ "การแข่งที่มีเดิมพัน" -> ส่งไป server ทันที
  const pickBrian = (id) => {
    socket.emit("useSkill", { tier: "ultimate", targets: [id] });
    setBrianSel(false);
  };
  ...
  useEffect(() => {
    // กุญแจรถไม่กินโควตาสกิลของเทิร์น จึงเช็คแค่เฟส/ล็อกมือ ไม่เช็ค me.skillUsed
  ...
  useEffect(() => {
    if (brianSel && (phase !== "PLAYING" || me?.skillUsed || done)) setBrianSel(false);
  }, [brianSel, phase, me?.skillUsed, done]);
  useEffect(() => {
  ...
  }, [supSel, phase, done, me?.supSkillUses, me?.supSkillMax]);
  useEffect(() => {
  ...
  if (vp.w < 768) {
    ...
    return (
      <div className="fixed inset-0 overflow-hidden flex flex-col">
        ...
        {brianSel && (
          <div className="shrink-0 text-center mt-1.5 text-hard">
            <span className="text-lg font-black text-echo-cyan animate-pulse">🏁 แตะเลือกคู่แข่งของ &quot;การแข่งที่มีเดิมพัน&quot;</span>
            <button onClick={() => { clickSound(); setBrianSel(false); }} className="ml-2 text-sm font-bold bg-black/60 rounded-full px-3 py-1 border border-white/30">ยกเลิก</button>
          </div>
        )}
        {supSel && (
          <div className="shrink-0 text-center mt-1.5 text-hard">
  ...
  return (
    <div className="fixed inset-0 overflow-hidden">
      ...
      {brianSel && (
        <div className="absolute top-[22%] left-1/2 -translate-x-1/2 z-40 text-center text-hard whitespace-nowrap">
          <span className="text-xl font-black text-echo-cyan animate-pulse bg-black/60 rounded-full px-5 py-1.5">🏁 คลิกเลือกคู่แข่งของ &quot;การแข่งที่มีเดิมพัน&quot;</span>
          <button onClick={() => { clickSound(); setBrianSel(false); }} className="ml-2 text-sm font-bold bg-black/60 rounded-full px-3 py-1 border border-white/30">ยกเลิก</button>
        </div>
      )}
      {supSel && (
