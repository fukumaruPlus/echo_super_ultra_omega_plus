...
**มหาเทพ อรชุน (patch 3.4)** — เทพผู้พิพากษาความชอบธรรม
- สกิลติดตัวอ่านประวัติ 2 อย่างที่**ไม่ใช่สถานะ** จึงไม่หมดอายุและล้างไม่ได้:
  `p.arjunaAttackers` (ใครเคยโจมตีอรชุน เก็บที่ตัวอรชุน) และ `p.hasKilled` (เคยสังหารคนอื่น เก็บที่ผู้สังหาร) ·
  `hasKilled` ตั้งใน `instantDeath()` จาก `effectSourceId` — ใช้ร่วมกันได้กับตัวละครในอนาคต
- **บันทึกผู้ลงมือที่ "ตอนเลือกเป้า" ไม่ใช่ตอนดาเมจลง** — `arjuna.onAttacked()` ต้องอยู่**ก่อน**ด่านหลบหลีก
  ทั้งหมดใน `doAttack()` ไม่งั้นคนที่ตีแล้วถูกหลบจะยังได้ส่วนลด -1 ของ "หัวใจที่เที่ยงธรรม" ต่อไปเรื่อยๆ
- Mahapralaya คิดดาเมจจาก `engine.attackPowerAgainst` (= `estimateAttackOn` ท่อเดียวกับคอนเนอร์)
  จึงรวมสกิลติดตัว/สังหารโลกาให้เอง แต่**ไม่รวมตัวคูณฝั่งรับ** (เปราะบางที่ท่านี้เพิ่งแจกจึงไม่ทำให้ดาเมจของตัวเองบวม)
- **Mahapralaya ล้างดีบัฟให้เป้าหมายที่ถูกโจมตี "คนละ 1 ขั้น" หลังลงดาเมจ** (ไม่ใช่ล้างทั้งหมดทีเดียว)
  — ลำดับสำคัญ: "สังหารโลกา" คิดดาเมจจากจำนวนดีบัฟของเป้าหมาย ถ้าล้างก่อนลงดาเมจ ท่าจะไปลดพลังของตัวเอง
  · การล้างนี้เป็น**ราคาที่จ่าย**เพื่อไม่ให้ท่าสะสมดีบัฟให้ตัวเองตีแรงขึ้นเรื่อยๆ ไม่ใช่ส่วนหนึ่งของดาเมจ
  · ใช้ `engine.cleanseOneStep()` ซึ่งเป็นของกลางใน `_universal_status.js` (ผู้วิงวอน Prayer ใช้ตัวเดียวกัน)
- ลำดับตามสเปคคือ แจกเปราะบาง -> วีดีโอ -> ดาเมจ จึงแยกเป็น `startPralaya()` (ตอนกด) กับ
  `applyPralaya()` (เรียกจาก `pausePlayingForCutscene` หลังวีดีโอจบ + ตาข่ายสำรองถ้าไม่ได้เข้าเส้นทางคัตซีน)
...
- จังหวะเดียวที่ต้องดักคือ **ตอนไอดอลเลือดหมด** (`tryIdolDown` เรียกจาก `instantDeath` แบบเดียวกับ
  `arjuna.tryRevive`) → สลับเจ้าของหลอดเป็นโปรดิวเซอร์ · `maxHpOf`/`maxArmorOf` คืน 5/3 หรือ 3/0 ตามธง
...
- **เกราะ Mark 42** (`characters/_mark42.js` — ระบบกลาง ไม่ใช่ตัวละคร · 15 เหรียญ · โอกาส/เพดานต่อรอบเท่าปืน) ใครก็ใส่ได้ยกเว้น ORT
