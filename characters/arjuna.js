...
module.exports = {
  ...
  applyPralaya(engine, p) {
    ...
    for (const o of engine.alivePlayers()) {
      ...
      if (!o.alive) { engine.log(`💀 ${o.name} ถูกมหาประลัยกลืนหายไป!`); continue; }
      // มหาประลัยชำระล้าง: ทุกคนที่ถูกท่านี้โจมตี ได้รับการล้างดีบัฟ "คนละ 1 ขั้น" (ไม่ใช่ล้างทั้งหมดทีเดียว)
      //  ทำ "หลังลงดาเมจ" โดยตั้งใจ — ดาเมจของ "สังหารโลกา" คิดจากจำนวนดีบัฟที่เป้าหมายมีอยู่
      //  ถ้าล้างก่อน ท่าจะไปลดพลังของตัวเอง · การล้างนี้คือราคาที่จ่าย ไม่ใช่ส่วนหนึ่งของการทำดาเมจ
      const washed = engine.cleanseOneStep(o);
      if (washed > 0) engine.log(`🌊✨ มหาประลัยชำระล้าง ${o.name} — ล้างดีบัฟให้ 1 ขั้น`);
    }
  },
};
